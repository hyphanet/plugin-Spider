...
import plugins.Spider.org.garret.perst.impl.Bitmap;
import plugins.Spider.org.garret.perst.impl.Bytes;
import plugins.Spider.org.garret.perst.impl.Page;
...
public class CompressedReadWriteFile implements IFile {
    public void write(long pageAddr, byte[] buf) {
        try {
            ...
            if (pageAddr != 0) {
                ...
                int newPageBitSize = (newPageSize + ALLOCATION_QUANTUM - 1) >>>
    ...
    public CompressedReadWriteFile(String dataFilePath, String cipherKey) {
        this(dataFilePath, dataFilePath + ".map", dataFilePath + ".log", 8 * 1024 * 1024,
             1024 * 1024, 1024 * 1024, false, false, cipherKey);
    }
    ...
    public CompressedReadWriteFile(String dataFilePath,  //
        ...
        try {
            ...
            pageIndexBuffer = pageIndexChan.map(readOnly
                    ? FileChannel.MapMode.READ_ONLY : FileChannel.MapMode.READ_WRITE, 0,  // position
                    pageIndexSize);
            ...
            if ( !readOnly) {
                ...
                performRecovery();
                bitmapExtensionQuantum = (int) (dataFileExtensionQuantum >>>
                        (ALLOCATION_QUANTUM_LOG + 3));
                bitmap =
                    new byte[(int) (dataChan.size() >>> (ALLOCATION_QUANTUM_LOG + 3)) + bitmapExtensionQuantum];
    ...
    long allocate(int bitSize) {
        ...
        if (pos < 0) {
            pos = Bitmap.allocate(bitmap, bitmapStart, Bitmap.locateHoleEnd(bitmap, bitmapPos),
                                  bitSize);
            ...
            if (pos < 0) {
                ...
                pos = Bitmap.allocate(newBitmap, Bitmap.locateBitmapEnd(newBitmap, bitmap.length),
    ...
    static class PageMap implements Iterable<PageMap.Entry> {
        ...
        public PageMap(int initialCapacity) {
            for (tableSizePrime = 0; primeNumbers[tableSizePrime] < initialCapacity;
                    tableSizePrime++);
