...
public class RtreeR2<T extends IPersistent> extends PersistentCollection<T>
    ...
    public void remove(RectangleR2 r, T obj) {
        ...
        for (int i = reinsertList.size(); --i >= 0; ) {
            ...
            for (int j = 0, n = p.n; j < n; j++) {
                RtreeR2Page q = root.insert(getStorage(), p.b[j], p.branch.get(j),
                                            height - reinsertLevel);
        ...
        private boolean gotoFirstItem(int sp, RtreeR2Page pg) {
            for (int i = 0, n = pg.n; i < n; i++) {
                if (r.intersects(pg.b[i])) {
                    if ((sp + 1 == height) ||
                            gotoFirstItem(sp + 1, (RtreeR2Page) pg.branch.get(i))) {
                        pageStack[sp] = pg;
                        posStack[sp] = i;
