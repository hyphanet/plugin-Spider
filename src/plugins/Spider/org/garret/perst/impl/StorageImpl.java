...
import plugins.Spider.org.garret.perst.fulltext.*;
...
public class StorageImpl implements Storage {
    ...
    final int getBitmapPageId(int i) {
        return (i < dbBitmapPages)
               ? dbBitmapId + i : header.root[1 - currIndex].bitmapExtent + i - bitmapExtentBase;
    }
    ...
    final void setPos(int oid, long pos) {
        synchronized (objectCache) {
            dirtyPagesMap[oid >>> (dbHandlesPerPageBits + 5)] |= 1 <<
                    ((oid >>> dbHandlesPerPageBits) & 31);
        ...
        synchronized (objectCache) {
            ...
            while (true) {
                    ...
                    for (i = firstPage; i < lastPage; i++) {
                        ...
                        while (offs < Page.pageSize) {
                            ...
                            if (holeBitSize + Bitmap.firstHoleSize[mask] >= objBitSize) {
                                ...
                                pos = (((long) i * Page.pageSize + offs) * 8 + holeBitOffset) <<
                                      dbAllocationQuantumBits;
                ...
                if (firstPage == 0) {
                    ...
                    int oldIndexSize = 0;
                    long oldIndex = 0;
                    int morePages = (int) ((extension +
                                            Page.pageSize * (dbAllocationQuantum * 8 - 1) -
                                            1) / (Page.pageSize * (dbAllocationQuantum * 8 - 1)));
                    ...
                    if ((i <= dbBitmapPages) && (i + morePages > dbBitmapPages)) {
                        ...
                        if (oldIndexSize <= currIndexSize + dbLargeBitmapPages - dbBitmapPages) {
                            ...
                            do {
                                ...
                                if (newIndexSize < 0) {
                                    ...
                                    if (newIndexSize <
                                            currIndexSize + dbLargeBitmapPages - dbBitmapPages) {
                                        throw new StorageError(StorageError.NOT_ENOUGH_SPACE);
                                    }
                            ...
                            if (size + newIndexSize * 8L > extensionQuantum) {
                                ...
                                morePages = (int) ((extension +
                                                    Page.pageSize * (dbAllocationQuantum * 8 - 1) -
                                                    1) / (Page.pageSize *
                                                    (dbAllocationQuantum * 8 - 1)));
                            }
                            ...
                            fillBitmap(pos + (skip >> 3) +
                                       (long) morePages *
                                       (Page.pageSize / dbAllocationQuantum / 8), newIndexSize >>>
                                           dbAllocationQuantumBits);
                        ...
                        System.arraycopy(bitmapPageAvailableSpace, 0, newBitmapPageAvailableSpace,
                                         0, dbBitmapPages);
                    ...
                    fillBitmap(pos + (skip >> 3),
                               morePages * (Page.pageSize / dbAllocationQuantum / 8));
                    j = i;
    ...
    final void cloneBitmap(long pos, long size) {
        synchronized (objectCache) {
            if (insideCloneBitmap) {
                ...
                while (true) {
                    long quantNo = pos >>> dbAllocationQuantumBits;
                    int objBitSize = (int) ((size + dbAllocationQuantum - 1) >>>
                                            dbAllocationQuantumBits);
                    int pageId = (int) (quantNo >>> (Page.pageSizeLog + 3));
                    int offs = (int) (quantNo & (Page.pageSize * 8 - 1)) >> 3;
                    int bitOffs = (int) quantNo & 7;
    ...
    public synchronized void open(IFile file, int pagePoolSize) {
        ...
        if (header.databaseFormatVersion == 0) {  // database not initialized
            ...
            int bitmapIndexSize = ((dbBitmapId + dbBitmapPages) * 8 + Page.pageSize - 1)
                                  & ~(Page.pageSize - 1);
        ...
        int bitmapSize = (header.root[1 - currIndex].bitmapExtent == 0)
                         ? dbBitmapPages : dbLargeBitmapPages;
    ...
    void reloadScheme() {
        ...
        classDescMap.put(ClassDescriptor.class, new ClassDescriptor(this, ClassDescriptor.class));
    ...
    public synchronized void backup(OutputStream out) throws java.io.IOException {
        ...
        int nIndexPages = (int) ((header.root[curr].indexSize + dbHandlesPerPage - 1) /
                                 dbHandlesPerPage);
        ...
        long newFileSize = (long) (nPagedObjects + nIndexPages * 2 + 1) * Page.pageSize +
                           totalRecordsSize;
        ...
        newHeader.root[0].shadowIndexSize = newHeader.root[0].indexSize =
            newHeader.root[1].shadowIndexSize = newHeader.root[1].indexSize =
            nIndexPages * dbHandlesPerPage;
        ...
        newHeader.root[0].classDescList = newHeader.root[1].classDescList =
            header.root[curr].classDescList;
        newHeader.root[0].bitmapExtent = newHeader.root[1].bitmapExtent =
            header.root[curr].bitmapExtent;
        ...
        for (i = 0; i < nObjects; i++) {
            ...
            if (((int) pos & (dbFreeHandleFlag | dbPageObjectFlag)) == dbPageObjectFlag) {
                if ((oids[i] < dbBitmapId + dbBitmapPages) ||
                    ...
                    } else if (mappedSpace + Page.pageSize * 8 * dbAllocationQuantum <=
                               newFileSize) {
    ...
    public <K extends Comparable, V extends IPersistent> IPersistentMap<K,
            V> createMap(Class keyType) {
        return createMap(keyType, 4);
    }

    public <K extends Comparable, V extends IPersistent> IPersistentMap<K,
            V> createMap(Class keyType, int initialSize) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }
    ...
    public synchronized <T extends IPersistent> Index<T> createIndex(Class keyType,
            boolean unique) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }
    ...
    public synchronized <T extends IPersistent> Index<T> createIndex(Class[] keyTypes,
            boolean unique) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }
    ...
    public synchronized <T extends IPersistent> MultidimensionalIndex<T> createMultidimensionalIndex(
    ...
    public <T extends IPersistent> FieldIndex<T> createFieldIndex(Class type, String fieldName,
            boolean unique) {
        return this.<T>createFieldIndex(type, fieldName, unique, false);
    }

    public synchronized <T extends IPersistent> FieldIndex<T> createFieldIndex(Class type,
        ...
        FieldIndex<T> index =
            ...
            ? alternativeBtree
              ...
              : (FieldIndex<T>) new BtreeCaseInsensitiveFieldIndex<T>(type, fieldName,
                  unique) : alternativeBtree
    ...
    public <T extends IPersistent> FieldIndex<T> createFieldIndex(Class type, String[] fieldNames,
            boolean unique) {
        ...
        FieldIndex<T> index =
            ...
            ? alternativeBtree
              ? (FieldIndex<T>) new AltBtreeCaseInsensitiveMultiFieldIndex(type, fieldNames,
                  unique) : (FieldIndex<T>) new BtreeCaseInsensitiveMultiFieldIndex(type,
                      fieldNames, unique) : alternativeBtree
    ...
    public synchronized <T extends IPersistent> Index<T> createRandomAccessIndex(Class keyType,
            boolean unique) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }
    ...
    public synchronized <T extends IPersistent> Index<T> createRandomAccessIndex(Class[] keyTypes,
            boolean unique) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }
    ...
    public <T extends IPersistent> FieldIndex<T> createRandomAccessFieldIndex(Class type,
            String fieldName, boolean unique) {
        return this.<T>createRandomAccessFieldIndex(type, fieldName, unique, false);
    }

    public synchronized <T extends IPersistent> FieldIndex<T> createRandomAccessFieldIndex(
            Class type, String fieldName, boolean unique, boolean caseInsensitive) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }

        FieldIndex<T> index = caseInsensitive
                              ? (FieldIndex) new RndBtreeCaseInsensitiveFieldIndex<T>(type,
                                  fieldName, unique) : (FieldIndex) new RndBtreeFieldIndex<T>(type,
                                      fieldName, unique);
    ...
    public synchronized <T extends IPersistent> FieldIndex<T> createRandomAccessFieldIndex(
            Class type, String[] fieldNames, boolean unique, boolean caseInsensitive) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }

        FieldIndex<T> index = caseInsensitive
                              ? (FieldIndex) new RndBtreeCaseInsensitiveMultiFieldIndex(type,
                                  fieldNames,
                                  unique) : (FieldIndex) new RndBtreeMultiFieldIndex(type,
                                      fieldNames, unique);
    ...
    public <T extends IPersistent & Comparable> SortedCollection<T> createSortedCollection(
            boolean unique) {
        if ( !opened) {
            throw new StorageError(StorageError.STORAGE_NOT_OPENED);
        }
    ...
    public synchronized HashMap getMemoryDump() {
        synchronized (objectCache) {
            ...
            int bitmapSize = (int) (header.root[currIndex].size >>>
                                    (dbAllocationQuantumBits + 5)) + 1;
            boolean existsNotMarkedObjects;
            long pos;
            int i, j, n;
            ...
            if (rootOid != 0) {
                        ...
                        if (greyBitmap[i] != 0) {
                            ...
                            for (j = 0; j < 32; j++) {
                                if ((greyBitmap[i] & (1 << j)) != 0) {
                                    ...
                                    int alignedSize = (objSize + dbAllocationQuantum - 1)
                                    ...
                                    if (typeOid != 0) {
                                        ...
                                        if (Btree.class.isAssignableFrom(desc.cls)) {
                                            ...
                                            if (FieldIndex.class.isAssignableFrom(desc.cls)) {
                                                ...
                                                fieldIndexUsage.totalSize +=
                                                    (long) nPages * Page.pageSize + objSize;
                                                fieldIndexUsage.allocatedSize += (long) nPages *
                                                        Page.pageSize + alignedSize;
                                            } else {
                                                indexUsage.nInstances += 1;
                                                indexUsage.totalSize += (long) nPages *
                                                                        Page.pageSize + objSize;
                                            ...
                                            if (desc.hasReferences) {
                                                markObject(pool.get(pos), ObjectHeader.sizeof,
                                                           desc);
                                            }
                                        }
                                    } else {
                ...
                if (header.root[currIndex].bitmapExtent != 0) {
                    system.allocatedSize =
                        getBitmapUsedSpace(dbBitmapId, dbBitmapId + dbBitmapPages) +
                        getBitmapUsedSpace(header.root[currIndex].bitmapExtent + dbBitmapPages -
                                           bitmapExtentBase, header.root[currIndex].bitmapExtent +
                                               header.root[currIndex].bitmapEnd - dbBitmapId -
                                               bitmapExtentBase);
                } else {
        ...
        for (int i = 0, n = all.length; i < n; i++) {
            ...
            switch (fd.type) {
                ...
                case ClassDescriptor.tpCustom : {
                    try {
                        ByteArrayInputStream in = new ByteArrayInputStream(obj, offs,
                                                      obj.length - offs);
    ...
    protected IPersistent unswizzle(int oid, Class cls, boolean recursiveLoading) {
        ...
        if ((cls == Persistent.class) || (desc = findClassDescriptor(cls)) == null ||
                desc.hasSubclasses) {
            long pos = getPos(oid);
            int offs = (int) pos & (Page.pageSize - 1);
            ...
            if ((f == null) || (obj == null)) {
                switch (fd.type) {
                    ...
                    case ClassDescriptor.tpArrayOfBoolean :
                        ...
                        continue;
                    case ClassDescriptor.tpCustom : {
                        ByteArrayInputStream in = new ByteArrayInputStream(body, offs,
                                                      body.length - offs);
                ...
                switch (fd.type) {
                    ...
                    case ClassDescriptor.tpObject : {
                        provider.set(f, obj,
                                     unswizzle(Bytes.unpack4(body, offs), f.getType(),
                                               recursiveLoading));
                        offs += 4;
                        ...
                        if (len >= 0) {
                            ...
                            switch (-2 - len) {
                                ...
                                case ClassDescriptor.tpLong :
                                    ...
                                    break;
                                case ClassDescriptor.tpFloat :
                                    val = new Float(Float.intBitsToFloat(Bytes.unpack4(body,
                                            offs)));
                                    offs += 4;

                                    break;
                                case ClassDescriptor.tpDouble :
                                    val = new Double(Double.longBitsToDouble(Bytes.unpack8(body,
                                            offs)));
                                    offs += 8;
                                ...
                                case ClassDescriptor.tpDate :
                                    ...
                                    break;
                                case ClassDescriptor.tpObject :
                                    val = unswizzle(Bytes.unpack4(body, offs), Persistent.class,
                                                    recursiveLoading);
                                    offs += 4;
                            }
                    ...
                    case ClassDescriptor.tpArrayOfObject :
                        ...
                        if (len < 0) {
                            ...
                            for (int j = 0; j < len; j++) {
                                arr[j] = unswizzle(Bytes.unpack4(body, offs), elemType,
                                                   recursiveLoading);
                                offs += 4;
                            }
                    ...
                    case ClassDescriptor.tpArrayOfRaw :
                        ...
                        if (len < 0) {
                            ...
                            for (int j = 0; j < len; j++) {
                                ...
                                if (rawlen >= 0) {
                                    ByteArrayInputStream bin = new ByteArrayInputStream(body, offs,
                                                                   rawlen);
                                    ObjectInputStream in = new PersistentObjectInputStream(bin);
    ...
    final int packValue(Object value, int offs, ByteBuffer buf, boolean finalized)
            throws Exception {
        if (value == null) {
            buf.extend(offs + 4);
    ...
    final int packObject(Object obj, ClassDescriptor desc, int offs, ByteBuffer buf,
                         IPersistent po, boolean finalized)
            throws Exception {
    ...
    public CustomAllocator createBitmapAllocator(int quantum, long base, long extension,
            long limit) {
