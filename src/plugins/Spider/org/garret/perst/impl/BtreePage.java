...
class BtreePage {
    ...
    final static int compare(Key key, Page pg, int i) {
        ...
        switch (key.type) {
            ...
            case ClassDescriptor.tpChar :
                return (char) key.ival -
                       (char) Bytes.unpack2(pg.data, BtreePage.firstKeyOffs + i * 2);
    ...
    static boolean find(StorageImpl db, int pageId, Key firstKey, Key lastKey, Btree tree,
            ...
            if (tree.type == ClassDescriptor.tpString) {
                ...
                if (lastKey != null) {
                    if (height == 0) {
                        ...
                        do {
                            if ( !find(db, getKeyStrOid(pg, l), firstKey, lastKey, tree, height,
                                       result)) {
                                return false;
                            }
                ...
                if (lastKey != null) {
                    if (height == 0) {
                        ...
                        do {
                            if ( !find(db, getReference(pg, maxItems - 1 - l), firstKey, lastKey,
                                       tree, height, result)) {
                                return false;
                            }
                ...
                if (height == 0) {
                    ...
                    do {
                        if ( !find(db, getReference(pg, maxItems - 1 - l), firstKey, lastKey, tree,
                                   height, result)) {
                            return false;
                        }
                    } while (++l <= n);
    ...
    static int insert(StorageImpl db, int pageId, Btree tree, BtreeKey ins, int height,
        ...
        try {
            if (tree.type == ClassDescriptor.tpString) {
                ...
                /* insert before e[r] */
                if (--height != 0) {
                    result = insert(db, getReference(pg, maxItems - r - 1), tree, ins, height,
                                    unique, overwrite);
                    Assert.that(result != Btree.op_not_found);
                ...
                if (merged_size > keySpace) {
                    ...
                    if (i > 0) {
                        ...
                        if (height != 1) {
                            ...
                            memcpy(a, getKeyStrOffs(a, k), pg, getKeyStrOffs(pg, r - 1), len * 2,
                                   1);
                        } else {
                        ...
                        for (j = 0; j < k; j++) {
                            ...
                            memcpy(a, getKeyStrOffs(a, j), b, getKeyStrOffs(b, bn - k + j),
                                   len * 2, 1);
                        }
    ...
    static void purge(StorageImpl db, int pageId, int type, int height) {
        if (--height != 0) {
            ...
            if ((type == ClassDescriptor.tpString) ||
                    (type == ClassDescriptor.tpArrayOfByte)) {  // page of strings
                while (--n >= 0) {
        ...
        try {
            ...
            if (--height != 0) {
                if ((type == ClassDescriptor.tpString) ||
                    ...
                    for (i = 0; i <= n; i++) {
                        pos = traverseForward(db, getReference(pg, maxItems - i - 1), type, height,
                                              result, pos);
                    }
                }
            } else {
                ...
                if ((type == ClassDescriptor.tpString) ||
                    ...
                    for (i = 0; i < n; i++) {
                        exporter.exportAssoc(getKeyStrOid(pg, i), pg.data,
                                             BtreePage.firstKeyOffs +
                                             BtreePage.getKeyStrOffs(pg,
                                                 i), BtreePage.getKeyStrSize(pg, i), type);
                    }
                } else {
                    for (i = 0; i < n; i++) {
                        exporter.exportAssoc(
                            getReference(pg, maxItems - 1 - i), pg.data,
