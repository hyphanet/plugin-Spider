...
public class FullTextIndexImpl extends PersistentResource implements FullTextIndex {
    ...
    private final void addReference(Document doc, String word, Occurrence[] occurrences, int from,
        ...
        for (int i = from; i < till; i++) {
            d.occurrences[i - from] = occurrences[i].position
                                      | (occurrences[i].kind << OCC_KIND_OFFSET);
        }
        ...
        int calculateWeight(FullTextQuery query) {
            switch (query.op) {
                ...
                case FullTextQuery.NEAR : {
                    ...
                    for (FullTextQuery q = ((FullTextQueryBinaryOp) query).right;
                            q.op == FullTextQuery.NEAR; q = ((FullTextQueryBinaryOp) q).right) {
                        shift += STRICT_MATCH_BONUS;
                    }

                    return (shift >= 32)
                           ? 0 : (calculateWeight(((FullTextQueryBinaryOp) query).left) >> shift);
                }
        ...
        int calculateEstimation(FullTextQuery query, int nResults) {
            switch (query.op) {
                ...
                case FullTextQuery.NEAR : {
                    ...
                    int right = calculateEstimation(((FullTextQueryBinaryOp) query).right,
                                    nResults);
            ...
            switch (query.op) {
                ...
                case FullTextQuery.STRICT_MATCH : {
                    ...
                    double density = frequency *
                                     Math.log(1 +
                                              (DENSITY_MAGIC * averageWords / d.nWordsInDocument));
