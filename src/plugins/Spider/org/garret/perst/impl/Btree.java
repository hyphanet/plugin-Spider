...
class Btree<T extends IPersistent> extends PersistentCollection<T> implements Index<T> {
    ...
    public ArrayList<T> prefixSearchList(String key) {
        ...
        if (root != 0) {
            BtreePage.prefixSearch((StorageImpl) getStorage(), root, key.toCharArray(), height,
                                   list);
        }
    ...
    public ArrayList<T> getList(Key from, Key till) {
        ...
        if (root != 0) {
            BtreePage.find((StorageImpl) getStorage(), root, checkKey(from), checkKey(till), this,
                           height, list);
        }
            ...
            if (BtreePage.getnItems(pg) == 0) {
                ...
                if (height != 1) {
                    newRoot = ((type == ClassDescriptor.tpString) ||
                               (type ==
                                ClassDescriptor.tpArrayOfByte)) ? BtreePage.getKeyStrOid(pg, 0)
    ...
    public <E> E[] toArray(E[] arr) {
        ...
        if (root != 0) {
            BtreePage.traverseForward((StorageImpl) getStorage(), root, type, height,
                                      (IPersistent[]) arr, 0);
        }
                ...
                if (nextKey == null) {
                    ...
                    while (true) {
                        ...
                        int oid = ((type == ClassDescriptor.tpString) ||
                                   (type ==
                                    ClassDescriptor.tpArrayOfByte)) ? BtreePage.getKeyStrOid(pg,
                                        pos) : BtreePage.getReference(pg,
                                            BtreePage.maxItems - 1 - pos);
        ...
        BtreeKey getCurrentKey(Page pg, int pos) {
            ...
            switch (type) {
                ...
                case ClassDescriptor.tpArrayOfByte :
                    ...
                    break;
                default :
                    key = new BtreeKey(null,
                                       BtreePage.getReference(pg, BtreePage.maxItems - 1 - pos));
                    key.extract(pg, BtreePage.firstKeyOffs + pos * ClassDescriptor.sizeof[type],
                                type);
            }
    ...
    public IterableIterator<Map.Entry<Object, T>> entryIterator(Object from, Object till,
            int order) {
