...
class AltBtree<T extends IPersistent> extends PersistentCollection<T> implements Index<T> {
    ...
    static abstract class BtreePage extends Persistent {
        ...
        static void memcpyData(BtreePage dst_pg, int dst_idx, BtreePage src_pg, int src_idx,
                               int len) {
            System.arraycopy(src_pg.getData(), src_idx, dst_pg.getData(), dst_idx, len);
        }

        static void memcpyItems(BtreePage dst_pg, int dst_idx, BtreePage src_pg, int src_idx,
                                int len) {
            System.arraycopy(src_pg.items.toRawArray(), src_idx, dst_pg.items.toRawArray(),
                             dst_idx, len);
        }
    ...
    public IterableIterator<T> prefixIterator(String prefix) {
        return iterator(new Key(prefix), new Key(prefix + Character.MAX_VALUE, false),
                        ASCENT_ORDER);
    }
