...
public class TtreePage extends Persistent {
    ...
    final boolean find(PersistentComparator comparator, Object minValue, int minInclusive,
        ...
        if (minValue != null) {
            if (-comparator.compareMemberWithKey(loadItem(0), minValue) >= minInclusive) {
                if (-comparator.compareMemberWithKey(loadItem(n - 1), minValue) >= minInclusive) {
                    if (right != null) {
                        return right.find(comparator, minValue, minInclusive, maxValue,
                                          maxInclusive, selection);
                    }
                ...
                while (r < n) {
                    if ((maxValue != null) &&
                            (comparator.compareMemberWithKey(loadItem(r), maxValue) >=
                             maxInclusive)) {
                        return false;
                    }
        ...
        if (left != null) {
            if ( !left.find(comparator, minValue, minInclusive, maxValue, maxInclusive,
                            selection)) {
                return false;
            }
        }
    ...
    final int insert(PersistentComparator comparator, IPersistent mbr, boolean unique,
                     PageReference ref) {
        load();
