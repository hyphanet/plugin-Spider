...
class PersistentMapImpl<K extends Comparable, V extends IPersistent> extends PersistentResource
    ...
    static class PersistentMapEntry<K extends Comparable, V extends IPersistent> extends Persistent
    ...
    static class PersistentMapComparator<K extends Comparable, V extends IPersistent>
    ...
    public V get(Object key) {
        if (index != null) {
            if (type == ClassDescriptor.tpRaw) {
                PersistentMapEntry<K, V> entry = ((SortedCollection<PersistentMapEntry<K,
                                                     V>>) index).get(key);
                ...
                if (size == BTREE_TRESHOLD) {
                    ...
                    if (type == ClassDescriptor.tpRaw) {
                        SortedCollection<PersistentMapEntry<K, V>> col =
                            getStorage().<PersistentMapEntry<K,
                                V>>createSortedCollection(new PersistentMapComparator<K, V>(),
                                    true);
    ...
    private V insertInSortedCollection(K key, V value) {
        SortedCollection<PersistentMapEntry<K, V>> col = (SortedCollection<PersistentMapEntry<K,
                                                             V>>) index;
            ...
            if (index != null) {
                if (type == ClassDescriptor.tpRaw) {
                    if (order == Index.ASCENT_ORDER) {
                        return new Iterator<Entry<K, V>>() {
                            private Iterator<PersistentMapEntry<K, V>> i =
                                ((SortedCollection<PersistentMapEntry<K,
                                    V>>) index).iterator(fromKey, toKey);
                            public boolean hasNext() {
                                return i.hasNext();
                            }
                            public Entry<K, V> next() {
                ...
                if (order == Index.ASCENT_ORDER) {
                    ...
                    return new Iterator<Entry<K, V>>() {
                        ...
                        public boolean hasNext() {
                            return (i + 1 < end) &&
                                   ((to == null) || ((Comparable[]) keys)[i + 1].compareTo(to) < 0);
                        }
                        public Entry<K, V> next() {
                            if ( !hasNext()) {
                                throw new NoSuchElementException();
                    ...
                    return new Iterator<Entry<K, V>>() {
                        ...
                        public boolean hasNext() {
                            return (i > 0) &&
                                   ((from == null) ||
                                    ((Comparable[]) keys)[i - 1].compareTo(from) >= 0);
                        }
                        public Entry<K, V> next() {
                            if ( !hasNext()) {
                                throw new NoSuchElementException();
    ...
    public K lastKey() {
        if (index != null) {
            if (type == ClassDescriptor.tpRaw) {
                ...
                return (K) ((Index<V>) index).entryIterator(null, null,
                        Index.DESCENT_ORDER).next().getKey();
            }
        } else {
            int size = values.size();
