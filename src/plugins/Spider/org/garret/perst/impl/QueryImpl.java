...
class Node {
    ...
    final static String typeNames[] = {
        "boolean", "integer", "real", "index variable", "list", "object", "string", "date",
        "array of boolean", "array of char", "array of byte", "array of short", "array of int",
        "array of long", "array of float", "array of double", "array of string", "array of object",
        "collection", "unknown", "any"
    };
    ...
    Object evaluateObj(FilterIterator t) {
        ...
        try {
            if ((target == null) && (t.containsElem != null)) {
                if ((m = QueryImpl.lookupMethod(t.containsElem.getClass(), methodName, profile)) !=
                        null) {
...
class ConvertAnyNode extends Node {
    public boolean equals(Object o) {
        return (o instanceof ConvertAnyNode) && super.equals(o) &&
               ((ConvertAnyNode) o).expr.equals(expr);
    }
...
class BinOpNode extends Node {
    ...
    Object evaluateObj(FilterIterator t) {
        ...
        if (lval instanceof Boolean) {
            switch (tag) {
                case opAnyAnd :
                    return ((Boolean) lval).booleanValue() &&
                           ((Boolean) right.evaluateObj(t)).booleanValue() ? Boolean.TRUE
                               : Boolean.FALSE;
                case opAnyOr :
                    return ((Boolean) lval).booleanValue() ||
                           ((Boolean) right.evaluateObj(t)).booleanValue() ? Boolean.TRUE
    ...
    static boolean areEqual(Object a, Object b) {
        ...
        if ((a instanceof Double) || (a instanceof Float) || (b instanceof Double) ||
                (b instanceof Float)) {
        ...
        try {
            if (base == null) {
                if (t.containsElem != null) {
                    ...
                    if ((m = QueryImpl.lookupMethod(cls, fieldName, QueryImpl.defaultProfile)) !=
                            null) {
                        this.m = m;
...
class ContainsNode extends Node implements Comparator {
    ...
    boolean evaluateBool(FilterIterator t) {
        ...
        if ((havingExpr != null) && ((withExpr != null) || !(collection instanceof Collection))) {
            ...
            t.containsArray =
                null;  // prevent reuse of the same array by nexted CONTAINS in with expression
        }
            ...
            if (method != null) {
                ...
                switch (type) {
                    case ClassDescriptor.tpBoolean :
                        diff = field.getBoolean(a)
                               ? field.getBoolean(b) ? 0 : 1 : field.getBoolean(b) ? -1 : 0;
...
public class QueryImpl<T> implements Query<T> {
    ...
    static Date parseDate(String source) {
        ...
        if (pos.getIndex() == 0) {
            throw new Error("Parse error for date \"" + source + "\" at position " +
                            pos.getErrorIndex());
        }
    ...
    final int scan() {
        ...
        switch (ch) {
            ...
            case '9' :
                ...
                if ((ch == '.') || (ch == 'e') || (ch == 'E')) {
                    while ((++p < eol) &&
                            (Character.isDigit(buf[p]) || (buf[p] == 'e') || (buf[p] == 'E') ||
                             (buf[p] == '.') ||
                             (((ch == 'e') || (ch == 'E')) &&
                              ((buf[p] == '-') || (buf[p] == '+')))));
                    pos = p;
            ...
            default :
                if (Character.isLetter(ch) || (ch == '$') || (ch == '_')) {
                    ...
                    while ((p < eol) &&
                            (Character.isLetterOrDigit(ch = buf[p]) || (ch == '$') ||
                             (ch == '_'))) {
                        p += 1;
                    }
    ...
    final Node comparison() {
        ...
        if ((cop == tknEq) || (cop == tknNe) || (cop == tknGt) || (cop == tknGe) ||
                (cop == tknLe) || (cop == tknLt) || (cop == tknBetween) || (cop == tknLike) ||
                (cop == tknNot) || (cop == tknIs) || (cop == tknIn)) {
            int rightPos = pos;
            boolean not = false;
            ...
            if (cop == tknIn) {
                ...
                if ((right.type != Node.tpList) &&
                        ((left.type == Node.tpAny) || (right.type == Node.tpAny))) {
                    ...
                    switch (right.type) {
                        ...
                        case Node.tpArrayBool :
                            if (left.type != Node.tpBool) {
                                throw new CompileError(
                                    "Incompatible types of IN operator operands", rightPos);
                            }
                        ...
                        case Node.tpStr :
                            if (left.type != Node.tpStr) {
                                throw new CompileError(
                                    "Left operand of IN expression hasn't string type", leftPos);
                            }
                ...
                if ((left.type == Node.tpAny) || (right.type == Node.tpAny) ||
                    ...
                    if (left.type == Node.tpInt) {
                        ...
                        throw new CompileError(
                            "operand of BETWEEN operator should be of integer, real or string type",
                            leftPos);
                    }
                    ...
                    left = new CompareNode(Node.opStrBetween, left, right, right2);
                } else if (left.type == Node.tpDate) {
                    if (right.type == Node.tpStr) {
                        right = str2date(right);
                    } else if (right.type != Node.tpDate) {
                        throw new CompileError(
                            "operands of BETWEEN operator should be of date type", rightPos);
                    }
                    ...
                    if (right2.type == Node.tpStr) {
                        ...
                        throw new CompileError(
                            "operands of BETWEEN operator should be of date type", andPos);
                    }

                    left = new CompareNode(Node.opDateBetween, left, right, right2);
                } else {
                    throw new CompileError(
                        "operands of BETWEEN operator should be of integer, real or string type",
                        rightPos);
                }
            } else if (cop == tknLike) {
                if (right.type == Node.tpUnknown) {
                ...
                if (lex == tknEscape) {
                    ...
                    left = new CompareNode(Node.opStrLikeEsc, left, right,
                ...
                if ((left.type == Node.tpAny) || (right.type == Node.tpAny)) {
                    ...
                    if (left.type == Node.tpInt) {
                        ...
                        throw new CompileError(
                            "operands of relation operator should be of intger, real or string type",
                            leftPos);
                    }
                    ...
                    if (right.type == Node.tpStr) {
                        ...
                        throw new CompileError(
                            "right opeerand of relation operator should be of date type", rightPos);
                    }
                    ...
                    if ((cop != tknEq) && (cop != tknNe)) {
                        throw new CompileError(
                            "Boolean variables can be checked only for equality", rightPos);
                    }

                    left = new BinOpNode(Node.tpBool, Node.opBoolEq + cop - tknEq, left, right);
                } else {
                    throw new CompileError(
                        "operands of relation operator should be of integer, real or string type",
                        rightPos);
                }
            }
        ...
        while ((lex == tknAdd) || (lex == tknSub)) {
            ...
            if ((left.type == Node.tpAny) || (right.type == Node.tpAny)) {
                ...
                if (left.type == Node.tpInt) {
                    ...
                    throw new CompileError(
                        "operands of arithmetic operators should be of integer or real type",
                        leftPos);
                }
                ...
                if (right.type == Node.tpInt) {
                    ...
                    throw new CompileError(
                        "operands of arithmetic operator should be of integer or real type",
                        rightPos);
                }

                left = new BinOpNode(Node.tpReal,
                                     (cop == tknAdd) ? Node.opRealAdd : Node.opRealSub, left,
                                     right);
            } else if ((left.type == Node.tpInt) && (right.type == Node.tpInt)) {
                left = new BinOpNode(Node.tpInt, (cop == tknAdd) ? Node.opIntAdd : Node.opIntSub,
                                     left, right);
            } else if ((left.type == Node.tpStr) && (right.type == Node.tpStr)) {
                if (cop == tknAdd) {
                ...
                throw new CompileError(
                    "operands of arithmentic operator should be of integer or real type", rightPos);
            }
    ...
    final Node multiplication() {
        ...
        while ((lex == tknMul) || (lex == tknDiv)) {
            ...
            if ((left.type == Node.tpAny) || (right.type == Node.tpAny)) {
                ...
                left = new BinOpNode(Node.tpReal,
                                     (cop == tknMul) ? Node.opRealMul : Node.opRealDiv, left,
                                     right);
            } else if ((left.type == Node.tpInt) && (right.type == Node.tpInt)) {
                left = new BinOpNode(Node.tpInt, (cop == tknMul) ? Node.opIntMul : Node.opIntDiv,
                                     left, right);
            } else {
                throw new CompileError(
                    "operands of arithmentic operator should be of integer or real type", rightPos);
    ...
    final Node component(Node base, Class cls) {
        ...
        if (lex == tknLpar) {
            ...
            if (unknownProfile) {
                if ( !cls.equals(Object.class) || (base != null) || (contains == null)) {
                    ...
                    return new InvokeAnyNode(base, ident, arguments,
                                             contains.containsExpr.getFieldName());
                }
            }
        }
        ...
        if (Object.class.equals(cls)) {
            ...
            throw new CompileError("No field or method '" + ident + "' in class " +
                                   ((cls == null)
                                    ? contains.containsFieldClass : cls).getName(), pos);
        }
    }
    ...
    final Node containsElement() {
        ...
        if ((arrClass == null) ||
                ( !arrClass.isArray() && !arrClass.equals(Object.class) &&
                  !(Collection.class.isAssignableFrom(arrClass)))) {
            throw new CompileError("Contains clause can be applied only to arrays or collections",
                                   p);
        }
            ...
            if (arrElemType.equals(Object.class)) {
                ...
                if (groupByField == null) {
                    ...
                    if (rt.equals(void.class) ||
                        ...
                        throw new CompileError("Result type " + rt +
                                               " of sort method should be comparable", p);
                    }
                } else {
                    Class type = groupByField.getType();

                    if ( !type.isPrimitive() && !Comparable.class.isAssignableFrom(type)) {
                        throw new CompileError("Order by field type " + type +
    ...
    final Node aggregateFunction(int cop) {
        ...
        if ((contains == null) ||
                ((contains.groupByField == null) && (contains.groupByMethod == null) &&
        ...
        switch (cop) {
            ...
            case tknAbs :
                ...
                if (expr.type == Node.tpInt) {
                    ...
                    throw new CompileError(
                        "ABS function can be applied only to integer or real expression", p);
                }
            case tknLength :
                expr = term();
                ...
                if (expr.type == Node.tpStr) {
                    ...
                    throw new CompileError(
                        "LENGTH function is defined only for arrays and strings", p);
                }
            case tknLower :
            ...
            case tknUpper :
                return field(new UnaryOpNode(Node.tpStr, Node.opStrUpper,
                                             checkType(Node.tpStr, term())));
            case tknInteger :
                return new UnaryOpNode(Node.tpInt, Node.opRealToInt,
                                       checkType(Node.tpReal, term()));
            case tknReal :
            ...
            case tknString :
                ...
                throw new CompileError(
                    "STRING function can be applied only to integer or real expression", p);
            case tknLpar : {
                expr = disjunction();
            ...
            case tknSub :
                ...
                if (expr.type == Node.tpInt) {
                    ...
                    throw new CompileError(
                        "Unary minus can be applied only to numeric expressions", p);
                }
            ...
            if ((key != null) && (cmp.right instanceof LiteralNode)) {
                ...
                switch (expr.tag) {
                    ...
                    case Node.opBoolEq : {
                        ...
                        if (value != null) {
                            return filter(index.iterator(value, value, Index.ASCENT_ORDER),
                                          filterCondition);
                        }
                    ...
                    case Node.opAnyGt : {
                        ...
                        if (value != null) {
                            return filter(index.iterator(value, null, Index.ASCENT_ORDER),
                                          filterCondition);
                        }
                    ...
                    case Node.opAnyLt : {
                        ...
                        if (value != null) {
                            return filter(index.iterator(null, value, Index.ASCENT_ORDER),
                                          filterCondition);
                        }
                ...
                switch (expr.tag) {
                    ...
                    case Node.opAnyBetween : {
                        ...
                        if ((value1 != null) && (value2 != null)) {
                            return filter(index.iterator(value1, value2, Index.ASCENT_ORDER),
                                          filterCondition);
                        }
                    ...
                    case Node.opStrLikeEsc : {
                        ...
                        char escape = (cmp.o3 != null)
                                      ? ((String) ((LiteralNode) cmp.o3).getValue()).charAt(0)
                                      : '\\';
                        int pref = 0;
                        ...
                        if (pref > 0) {
                            if (pref == pattern.length()) {
                                ...
                                return filter(index.prefixIterator(pattern.substring(0, pref)),
                                              condition);
                            }
                        }
                    }
