...
import plugins.Spider.org.garret.perst.impl.ClassDescriptor;
...
public class Database {
    ...
    private boolean addIndices(Table table, Class cls) {
        ...
        for (Field f : cls.getDeclaredFields()) {
            ...
            if (idx != null) {
                schemaUpdated |= createIndex(table, cls, f.getName(), idx.unique(),
                                             idx.caseInsensitive());
            }
        }
            ...
            for (Class c = cls; c != Object.class; c = c.getSuperclass()) {
                ...
                if (t == null) {
                    if ( !exclusiveLockSet) {
                        ...
                        // try to avoid deadlock caused by concurrent insertion of objects
                        metadata.unlock();
    ...
    private boolean createIndex(Table t, Class c, String key, boolean unique,
    ...
    public <T extends IPersistent> IterableIterator<T> getRecords(Class table, boolean forUpdate) {
        ...
        return new IteratorWrapper<T>((t == null)
                                      ? new LinkedList<T>().iterator() : t.extent.iterator());
    }
     ...
     */
    public FullTextSearchResult search(String query, String language, int maxResults,
            int timeLimit) {
        if (multithreaded) {
    ...
    static class Metadata extends PersistentResource {
        ...
        Metadata(Storage storage, Index index, FullTextSearchHelper helper) {
            ...
            fullTextIndex = (helper != null)
                            ? storage.createFullTextIndex(helper) : storage.createFullTextIndex();
        }
    ...
    static class Table extends Persistent {
        ...
        void setClass(Class cls) {
            ...
            for (Field f : cls.getDeclaredFields()) {
                FullTextIndexable idx =
                    (FullTextIndexable) f.getAnnotation(FullTextIndexable.class);
